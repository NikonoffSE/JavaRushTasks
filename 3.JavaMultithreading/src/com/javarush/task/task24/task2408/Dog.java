...
public class Dog implements Pet {
    ...
    public Sayable toSayable(final int i) {
        ...
        class SD extends SuperDog implements Sayable{
            String s = "";
            protected SimpleDateFormat formatter1 = new SimpleDateFormat("dd-MMM-yyyy E");
            @Override
            public String say() {
                StringBuilder sb = new StringBuilder(formatter1.format(new Date()).replaceAll("\\.", ""));
                if (sb.substring(7,8).equals("-")) sb.deleteCharAt(6);
                sb.replace(12,13,sb.substring(12,13).toUpperCase());
                for (int j = 0; j < i; j++) {
                    s = s + "а";
                }
                if (i<1) {
                   ...
                   if (i>=1) return getSuperQuotes() + name + getSuperQuotes() + " лает г" + s + "в! " + formatter.format(new Date()); //sb.toString();
                }
                return null;
            }
        }
